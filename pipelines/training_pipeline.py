...
def ml_pipeline():
    ...
    log_transformed = feature_engineering_step(df=filled_data, strategy="log")

    # outline detection step
    clean_data = outlier_detection_step(df=log_transformed, strategy="zscore", method="remove")

    # data splitting step
    X_train, X_test, y_train, y_test = data_splitter_step(df=clean_data, target_column="Attrition")
